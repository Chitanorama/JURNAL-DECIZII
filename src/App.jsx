...
const Inp=({label,...p})=><div style={{marginBottom:12}}>{label&&<label style={{display:"block",fontSize:12,color:"#888",marginBottom:3,fontWeight:500}}>{label}</label>}<input style={{width:"100%",boxSizing:"border-box",background:"#fff",border:"1px solid #ddd",borderRadius:8,padding:"8px 10px",fontSize:14}} {...p}/></div>;
const Sel=({label,children,...p})=><div style={{marginBottom:12}}>{label&&<label style={{display:"block",fontSize:12,color:"#888",marginBottom:3,fontWeight:500}}>{label}</label>}<select style={{width:"100%",boxSizing:"border-box",background:"#fff",border:"1px solid #ddd",borderRadius:8,padding:"8px 10px",fontSize:14}} {...p}>{children}</select></div>;
const Txt=({label,locked,...p})=><div style={{marginBottom:8}}>{label&&<label style={{display:"block",fontSize:12,color:"#888",marginBottom:3,fontWeight:500}}>{label}</label>}<textarea style={{width:"100%",boxSizing:"border-box",minHeight:68,resize:"vertical",fontSize:14,padding:"8px 10px",border:"1px solid #ddd",borderRadius:8,background:locked?"#f5f5f5":"#fff",cursor:locked?"not-allowed":"text"}} readOnly={!!locked} {...p}/></div>;
...
function AddableSel({label,value,onChange,options,onAdd,onDelete,canAdd}){
  ...
  return(<div style={{marginBottom:14}}>
    ...
    {adding&&<div style={{display:"flex",gap:6,marginTop:6}}>
      <input autoFocus placeholder="Denumire noua..." value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doAdd()} style={{flex:1,fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"6px 10px"}}/>
...
function LinksList({links,onChange}){
  ...
  return(<div style={{marginBottom:12}}>
    ...
    <input placeholder="Eticheta (optional)" value={nl} onChange={e=>setNl(e.target.value)} style={{width:"100%",boxSizing:"border-box",fontSize:13,marginBottom:4,border:"1px solid #ddd",borderRadius:7,padding:"6px 10px"}}/>
    <div style={{display:"flex",gap:6}}>
      <input placeholder="https://..." value={nu} onChange={e=>setNu(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} style={{flex:1,fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"6px 10px"}}/>
      <button onClick={add} style={{fontSize:13,padding:"0 12px",whiteSpace:"nowrap",border:"1px solid #ddd",borderRadius:7,cursor:"pointer"}}>+ Link</button>
    </div>
  </div>);
}
  ...
  return(<div style={{position:"fixed",inset:0,zIndex:200,display:"flex"}} onClick={onClose}>
    ...
    <div style={{width:"min(500px,100vw)",background:"#f0e8e4",borderLeft:"1.5px solid #ddd0ca",height:"100%",overflowY:"auto",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"16px 20px",borderBottom:"1px solid #ddd0ca",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#e8dcd8",flexShrink:0}}>
        ...
        <button onClick={onClose} style={{fontSize:13,padding:"4px 12px",border:"1px solid #ccc",borderRadius:6,cursor:"pointer",background:"#fff"}}>✕ Inchide</button>
      </div>
...
function TeamPicker({team,setTeam,users,nm,setNm}){
  ...
  return(<div style={{marginBottom:12}}>
    ...
    {users.length>0&&<select onChange={e=>{if(e.target.value&&!t.includes(e.target.value))setTeam([...t,e.target.value]);e.target.value="";}} style={{width:"100%",boxSizing:"border-box",fontSize:13,marginBottom:8,border:"1px solid #ddd",borderRadius:7,padding:"7px 10px"}}>
    ...
    <div style={{display:"flex",gap:6}}>
      <input placeholder="Sau adauga persoana noua..." value={nm} onChange={e=>setNm(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nm.trim()){setTeam([...t,nm.trim()]);setNm("");}}} style={{flex:1,fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"7px 10px"}}/>
      <button onClick={()=>{if(nm.trim()){setTeam([...t,nm.trim()]);setNm("");}}} style={{border:"1px solid #ddd",borderRadius:7,padding:"7px 12px",cursor:"pointer"}}>+ Adauga</button>
    </div>
  </div>);
}
  ...
  return(<div style={{padding:"2rem 1.5rem"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:12}}>
      ...
      {isAdmin&&<div style={{display:"flex",gap:8}}>
        <button onClick={onUsers} style={{fontSize:13,padding:"6px 14px",border:"1px solid #ddd",borderRadius:8,cursor:"pointer",background:"#fff"}}>Utilizatori</button>
    ...
    {projects.length===0
      ...
      :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14}}>
        {projects.map(p=><div key={p.id} onClick={()=>onSelect(p)} style={{background:"#fff",border:"1px solid #e8e6e1",borderRadius:12,padding:"18px 18px 14px",cursor:"pointer",transition:"box-shadow 0.15s"}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 12px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
          <div style={{width:36,height:36,borderRadius:8,background:"#534AB715",border:"1px solid #534AB730",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,marginBottom:12}}>🏠</div>
  ...
  return(<Drawer title="Utilizatori si acces" onClose={onClose}>
    ...
    {users.length>0&&<Card2><SecTitle2>Colegi existenti</SecTitle2>
      {users.map(u=><div key={u.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #f0eeeb"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          <button onClick={()=>{setUf({name:u.name,code:u.code,projectIds:u.projectIds||[]});setEuid(u.id);}} style={{fontSize:12,padding:"3px 10px",border:"1px solid #ddd",borderRadius:6,cursor:"pointer"}}>edit</button>
          <button onClick={()=>del(u.id)} disabled={loading} style={{fontSize:12,padding:"3px 10px",color:"#A32D2D",border:"1px solid #fcc",borderRadius:6,cursor:"pointer"}}>x</button>
        </div>
      </div>)}
    </Card2>}
  </Drawer>);
...
export default function App(){
  ...
  const loadGlobal=useCallback(async()=>{
    try{
      ...
      rows.forEach(r=>{if(r.id==="phases")setPhases(r.data);if(r.id==="specs")setSpecs(r.data);});
  ...
  const loadProjects=useCallback(async()=>{
    try{const rows=await dbGet("projects");return rows.map(r=>r.data);}
    catch(e){console.error(e);return[];}
  },[]);

  const loadDecisions=useCallback(async(projectId)=>{
    try{const rows=await dbGet("decisions",{project_id:projectId});return rows.map(r=>r.data);}
    catch(e){console.error(e);return[];}
  },[]);
  ...
  const handleLogin=async()=>{
    ...
    }catch(e){setLerr("Eroare de conexiune: "+e.message);}
    setLoading(false);
  };
  ...
  const addProject=async()=>{
    ...
    }catch(e){console.error("addProject error:",e);}
    setLoading(false);
  };
  ...
  if(!session)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f8f7f5",padding:"2rem",boxSizing:"border-box"}}>
      <div style={{width:"100%",maxWidth:380,textAlign:"center"}}>
        ...
        {lerr&&<div style={{fontSize:13,color:"#A32D2D",marginBottom:10,textAlign:"left",padding:"8px 12px",background:"#fef2f2",borderRadius:7,border:"1px solid #fcc"}}>{lerr}</div>}
  ...
  // PROJECT SELECTION
  if(!ap)return(
    <div style={{fontFamily:"system-ui,sans-serif",background:"#f8f7f5",minHeight:"100vh"}}>
      <div style={{background:"#fff",borderBottom:"1.5px solid #e0ddd8",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:52}}>
        ...
        <button onClick={logout} style={{fontSize:13,padding:"5px 12px",color:"#aaa",border:"1px solid #eee",borderRadius:6,cursor:"pointer",background:"#fff"}}>Iesi</button>
      </div>
      <ProjectCards
        projects={myProjects}
        onSelect={setAp}
      ...
      {drawer==="newProject"&&<Drawer title="Proiect nou" onClose={()=>setDrawer(null)}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:16}}>
          <button onClick={()=>setDrawer(null)} style={{padding:"7px 16px",border:"1px solid #ddd",borderRadius:8,cursor:"pointer"}}>Anuleaza</button>
    ...
    <div style={{fontFamily:"system-ui,sans-serif",background:"#f8f7f5",minHeight:"100vh"}}>
      <div style={{background:"#fff",borderBottom:"1.5px solid #e0ddd8",padding:"0 20px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:52,flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {isAdmin&&<button onClick={()=>{setPf(makeProj());setDrawer("newProject");}} style={{fontSize:13,padding:"5px 12px",border:"1px solid #ddd",borderRadius:7,cursor:"pointer",background:"#fff"}}>+ Proiect</button>}
            {isAdmin&&<button onClick={()=>setDrawer("users")} style={{fontSize:13,padding:"5px 12px",border:"1px solid #ddd",borderRadius:7,cursor:"pointer",background:"#fff"}}>Utilizatori</button>}
            {isAdmin&&<button onClick={openSettings} style={{fontSize:13,padding:"5px 12px",border:"1px solid #ddd",borderRadius:7,cursor:"pointer",background:"#fff"}}>Setari</button>}
            {!isClient&&<Btn1 onClick={openNew}>+ Decizie</Btn1>}
            <button onClick={logout} style={{fontSize:13,padding:"5px 12px",color:"#aaa",border:"1px solid #eee",borderRadius:7,cursor:"pointer",background:"#fff"}}>Iesi</button>
          </div>
        </div>
      </div>
      ...
      <div style={{padding:"20px"}}>
        ...
        <Card2 style={{padding:0,overflow:"hidden"}}>
          <div style={{padding:"12px 16px",borderBottom:"1px solid #e8e6e1",display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",background:"#fafafa"}}>
            <input placeholder="Cauta topic..." value={srch} onChange={e=>setSrch(e.target.value)} style={{flex:1,minWidth:140,fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"7px 10px"}}/>
            <select value={fph} onChange={e=>setFph(e.target.value)} style={{fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"7px 10px"}}>
              <option value="Toate">Toate fazele</option>
              {phases.map(p=><option key={p}>{p}</option>)}
            </select>
            <select value={fst} onChange={e=>setFst(e.target.value)} style={{fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"7px 10px"}}>
                  ...
                  <tr key={d.id} onClick={()=>{setDid(d.id);setCtxt("");setCname("");setCimgs([]);setDrawer("detail");}}
                    ...
                    <td style={{padding:"11px 16px",verticalAlign:"middle",fontSize:12,maxWidth:0}}>
                      {d.status==="Decizie luata"&&d.approvedBy
                        ...
                        :<span style={{color:d.decisionOwner?"#333":"#ccc",fontWeight:d.decisionOwner?500:400}}>{d.decisionOwner||"-"}</span>}
                    </td>
      ...
      {drawer==="detail"&&dd&&<Drawer title="Detalii topic" onClose={()=>{setDrawer(null);setDid(null);}}>
        <div style={{marginBottom:16}}>
          ...
          {dd.description&&<div style={{fontSize:14,lineHeight:1.6,marginBottom:8,whiteSpace:"pre-wrap",color:dd.title?"#555":"#333",fontWeight:dd.title?400:500}}>{dd.description}</div>}
        ...
        <Card2 style={{marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[["Creat de",dd.createdBy],["Decide",dd.decisionOwner],["Decizie luata de",dd.approvedBy],["Data deciziei",dd.dateDecision],["Inregistrat la",dd.dateAdded]].map(pair=>(
              <div key={pair[0]}><div style={{fontSize:11,color:"#aaa",marginBottom:2,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.04em"}}>{pair[0]}</div><div style={{fontSize:13,fontWeight:500,color:pair[1]?"#333":"#ccc"}}>{pair[1]||"-"}</div></div>
            ))}
          </div>
        </Card2>
        <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
          {!isClient&&(isAdmin||(dd.createdBy===session.name))&&<button onClick={()=>{setDrawer(null);openEdit(dd);}} style={{fontSize:13,padding:"6px 16px",border:"1px solid #ddd",borderRadius:8,cursor:"pointer"}}>Editeaza</button>}
          {canDelete(dd)&&<button onClick={()=>deleteDec(dd)} style={{fontSize:13,padding:"6px 16px",color:"#A32D2D",border:"1px solid #fcc",borderRadius:8,cursor:"pointer"}}>Sterge</button>}
          ...
          {isAdmin&&dd.status==="Decizie luata"&&<button onClick={()=>setDecStatus(dd,"In discutie")} style={{fontSize:13,padding:"6px 16px",color:"#888",border:"1px solid #ddd",borderRadius:8,cursor:"pointer"}}>Redeschide</button>}
        </div>
        <Card2>
          ...
          <div style={{borderTop:"1px solid #eee",paddingTop:12}}>
            <select value={cname} onChange={e=>setCname(e.target.value)} style={{width:"100%",boxSizing:"border-box",marginBottom:8,fontSize:13,border:"1px solid #ddd",borderRadius:7,padding:"7px 10px"}}>
              <option value="">- cine scrie? -</option>
              {projectTeam.map(m=><option key={m}>{m}</option>)}
            </select>
            <textarea value={ctxt} onChange={e=>setCtxt(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();addComment();}}} placeholder="Scrie un mesaj... (Enter pentru trimite)" style={{width:"100%",boxSizing:"border-box",minHeight:60,resize:"vertical",fontSize:13,padding:"8px 10px",border:"1px solid #ddd",borderRadius:8,background:"#fff",marginBottom:6}}/>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
          {!confirmDel
            ...
            :<div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={()=>setConfirmDel(false)} style={{fontSize:13,padding:"6px 14px",border:"1px solid #ddd",borderRadius:8,cursor:"pointer"}}>Anuleaza</button>
            </div>}
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{setDrawer(null);setConfirmDel(false);}} style={{padding:"7px 16px",border:"1px solid #ddd",borderRadius:8,cursor:"pointer"}}>Inchide</button>
